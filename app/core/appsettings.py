...
KEYS: dict[str, KeySpec] = {
    ...
    "registration_open": KeySpec(
        ...
        False,
        "注册入口对所有人可见",
        # ★★★ `B170` **重新定义了它**：★ 它管的是【**入口可见性**】，❌ 不是"彻底关掉注册" ——
        #   ★ `True` ⇒ 入口对所有人可见 + 可注册；
        #   ★★ `False` ⇒ 入口隐藏（前端不显示注册按钮），⚠ **但持邀请码者仍可注册** ✅
        #   ⚠★ 想**彻底停掉注册**请用 `paused`（★ 那才是重锤）
        "★ 一期默认【关】—— ★ 邀请制测试期（U4.7）：★ 入口对普通访客不显示，"
        "⚠ 但**持邀请码者仍可注册**（`B170` 改的语义，旧写法会让邀请码变成废纸）；"
        "★ 要彻底停掉注册用 `paused`",
    ),
    "login_open": KeySpec(
        "login_open",
